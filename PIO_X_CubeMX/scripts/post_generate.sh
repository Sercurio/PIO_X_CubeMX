...
# Déplacer les Drivers et Middlewares -> lib
if [ -d "$CUBE_DRIVERS_DIR" ]; then
    mv "$CUBE_DRIVERS_DIR" "$PIO_LIB_DEST/"
fi
if [ -d "$CUBE_MIDDLEWARES_DIR" ]; then
    mv "$CUBE_MIDDLEWARES_DIR" "$PIO_LIB_DEST/"
fi
...
# Copier le linker script
mv "$PIO_PROJECT_DIR"/*.ld "$PIO_PROJECT_DIR/" 2>/dev/null || true


echo "✅ Synchronisation terminée avec succès !"
echo "RAPPEL : Intégrez les appels d'initialisation depuis src/main.c.bak dans votre code."
echo "--------------------------------------------------------------------"